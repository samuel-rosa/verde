...
A última etapa da compilação de metadados de métodos consiste na identificação das unidades de medida das variáveis. A principal razão para isso está relacionada à origem dos dados. É possível que cada laboratório apresente os dados de uma mesma variável utilizando unidades de medida distintas. Quando isso acontece, temos em mãos variáveis operacionalmente distintas, mesmo que os laboratórios tenham empregado exatamente o mesmo procedimento analítico. Esse não é o caso das variáveis usadas como exemplo até aqui, sendo suficiente mover as unidades de medida para sua própria coluna (Tabela \@ref(tab:metodos-unidade-de-medida)). Do contrário, também seria necessário inserir linhas para identificar as novas variáveis e suas respectivas unidades de medida.
